		...
		<form action="<?php bloginfo('url') ?>/material/inquiry-003/conf" method="POST" class="mar_top40">
			<div class="form-center">
				...
				<div class="contact-item">
					...
					<input type="text" name="film_size" placeholder="<?php echo (get_current_blog_id()==1)?'縦  mm・横  mm':'' ?>" />
				</div>
				<div class="contact-item">
				...
				<div class="contact-item">
					...
					<input type="text" name="print_fix" placeholder="<?php echo (get_current_blog_id()==1)?'＊製造者記号　＊バーコード等':'' ?>" />
				</div>
				<div class="contact-item">
					<div class="title"><?php echo (get_current_blog_id()==1)?'可変印字情報':'Variable print information ' ?></div>
					<input type="text" name="print_change" placeholder="<?php echo (get_current_blog_id()==1)?'＊製造日　＊賞味期限　＊消費期限　＊ロット番号　＊トレーサビリティ記号など':'' ?>" />
				</div>
				<div class="contact-item">
