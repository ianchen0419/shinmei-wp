	...
	<div class="wrapper-size">
		...
		<form action="<?php bloginfo('url') ?>/material/inquiry-002/conf" method="POST" class="mar_top40">
			<div class="form-center">
				...
				<div class="contact-item">
					<div class="title"><?php echo (get_current_blog_id()==1)?'現在の印字・表示方法':'Current print and display method' ?></div>
					<input type="text" name="print_method" placeholder="<?php echo (get_current_blog_id()==1)?'手貼り、プレ印刷など':'' ?>" />
				</div>
				<div class="contact-item">
					<div class="title"><?php echo (get_current_blog_id()==1)?'印字対象の1日の使用量':'Daily usage for printing object' ?></div>
					<input type="text" name="use_amount" placeholder="<?php echo (get_current_blog_id()==1)?'何枚':'' ?>" />
				</div>
				<div class="contact-item">
					<div class="title"><?php echo (get_current_blog_id()==1)?'印字対象の材質':'Material of printing object' ?></div>
					<input type="text" name="print_material" placeholder="<?php echo (get_current_blog_id()==1)?'＊袋：ポリ、アルミ等　＊ラベル：コード紙、合成紙等　＊その他':'' ?>" />
				</div>
				<div class="contact-item">
					<div class="title"><?php echo (get_current_blog_id()==1)?'印字対象の機能':'Function of printing object' ?></div>
					<input type="text" name="print_function" placeholder="<?php echo (get_current_blog_id()==1)?'チェック付きなど':'' ?>" />
				</div>
				<div class="contact-item">
					<div class="title"><?php echo (get_current_blog_id()==1)?'印字対象のサイズ・厚さ':'Size and thickness of printing object' ?></div>
