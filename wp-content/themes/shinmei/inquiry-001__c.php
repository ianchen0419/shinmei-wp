		...
		<?php }?>
			<div class="form-center">
				<div class="contact-item">
					...
					<p>包装機用 超小型ダイレクトサーマルプリンター SCS</p>
				</div>
				...
				<div class="contact-item">
					<div class="title">包装する製品の種類</div>
					<input type="text" readonly disabled name="package_type" value="<?php echo $_POST['package_type'] ?>" />
				</div>
				<div class="contact-item">
					<div class="title">包装機明細</div>
					<input type="text" readonly disabled name="machine_detail" value="<?php echo $_POST['machine_detail'] ?>" />
				</div>
				<div class="contact-item less-checkbox">
					<div class="title">包装機ピロー種類</div>
					<label class="checkbox">
						<input type="checkbox" readonly disabled name="pillow_type[]" value="縦ピロー" <?php echo isset($_POST['pillow_type1'])?'checked':''; ?> />
						<i class="fa fa-check"></i>
						<span>縦ピロー</span>
					</label>
					<label class="checkbox">
						<input type="checkbox" readonly disabled name="pillow_type[]" value="横ピロー" <?php echo isset($_POST['pillow_type2'])?'checked':''; ?> />
						<i class="fa fa-check"></i>
						<span>横ピロー</span>
					</label>
				</div>
				<div class="contact-item less-checkbox">
					<div class="title">包装機稼働方法</div>
					<label class="checkbox">
						<input type="checkbox" readonly disabled name="operate_method[]" value="連続式" <?php echo isset($_POST['operate_method1'])?'checked':''; ?> />
						<i class="fa fa-check"></i>
						<span>連続式</span>
					</label>
					<label class="checkbox">
						<input type="checkbox" readonly disabled name="operate_method[]" value="間欠式" <?php echo isset($_POST['operate_method2'])?'checked':''; ?> />
						<i class="fa fa-check"></i>
						<span>間欠式</span>
					</label>
				</div>
				<div class="contact-item less-checkbox">
					<div class="title">印字内容</div>
					<label class="checkbox">
						<input type="checkbox" readonly disabled name="print_content[]" value="日付専用" <?php echo isset($_POST['print_content1'])?'checked':''; ?> />
						<i class="fa fa-check"></i>
						<span>日付専用</span>
					</label>
					<label class="checkbox">
						<input type="checkbox" readonly disabled name="print_content[]" value="製造者、原材料などその他の一括表示" <?php echo isset($_POST['print_content2'])?'checked':''; ?> />
						<i class="fa fa-check"></i>
						<span>製造者、原材料などその他の一括表示</span>
					</label>
				</div>
				<div class="contact-item">
					<div class="title">印字ピッチ</div>
					<input type="text" readonly disabled name="print_pitch" value="<?php echo $_POST['print_pitch'] ?>" />
				</div>
				<div class="contact-item">
					<div class="title">印字ショット数</div>
